...
/** Le prix d'une formule, tel que la BASE le donne — jamais écrit en dur ici. */
async function prixOffre(code: string): Promise<number> {
  const offre = await lireOffreParCode(code, 'international');
  if (!offre) throw new Error(`Formule ${code} absente ou sans prix.`);
  return offre.montant;
}

describe('la préparation d’une souscription', () => {
  it('rend devise, zone et essai — tous décidés par le SERVEUR', async () => {
    const preparation = await preparerSouscription({ userId: abonne.id, email: abonne.email }, 'lecture');
  ...
  it('les montants viennent de la BASE, plus de l’environnement', async () => {
    // Depuis la migration 0068, les formules sont administrées : leurs prix
    // sont des lignes de `plan_prices`, et non deux variables d'un fichier
    // `.env` qu'aucun écran ne pouvait modifier (§4.3 F12 bis).
    const mensuel = await prixOffre('lecture-mensuel');
    const annuel = await prixOffre('lecture-annuel');

    expect(mensuel).toBeGreaterThan(0);
    expect(annuel).toBeGreaterThan(mensuel);
  });
  ...
  it('est la MÊME source que la route d’ouverture — pas une seconde grille', async () => {
    ...
    await souscrireSimule(
      postJson('/api/abonnement-simule', { offre: 'lecture-annuel', issue: 'reussi' }, {
    ...
    expect(Number(enBase?.montant)).toBe(await prixOffre('lecture-annuel'));
...
describe('le règlement simulé d’une souscription', () => {
  it('exige d’être connecté', async () => {
    const reponse = await souscrireSimule(
      postJson('/api/abonnement-simule', { offre: 'lecture-mensuel', issue: 'reussi' }),
    );
  ...
  it('un ÉCHEC ne crée rien, et n’émet aucun événement', async () => {
    ...
    const reponse = await souscrireSimule(
      postJson('/api/abonnement-simule', { offre: 'lecture-mensuel', issue: 'echoue' }, {
        jeton: abonne.accessToken,
      }),
    );
    ...
    expect(await abonnementCourant(abonne.id, 'lecture')).toBeNull();
  });
...
describe('AUCUN TARIF NE VIENT DU CLIENT', () => {
  it('un montant, une zone ou un essai soumis sont IGNORÉS', async () => {
    ...
    expect(Number(enBase?.montant)).toBe(await prixOffre('lecture-mensuel'));
    expect(enBase?.devise).toBe('EUR');
    expect(enBase?.zone).toBe('international');
    // Non pas « différent de 3650 » : la valeur EXACTE que le serveur a décidée.
    expect(enBase?.jours_essai).toBe(preparation.joursEssai);
  });

  it('refuse une offre inconnue', async () => {
    // 404, et non plus 400 : le code d'une formule n'est plus une valeur close
    // que Zod énumérerait — l'éditeur en crée à volonté (§4.3 F12 bis). C'est
    // `offre_par_code` qui refuse, et il refuse d'un même geste le code
    // inventé, la formule désactivée et la formule sans prix dans la zone.
    const reponse = await souscrireSimule(
      postJson('/api/abonnement-simule', { offre: 'a-vie', issue: 'reussi' }, {
        jeton: abonne.accessToken,
      }),
    );
  ...
  it('refuse une issue inconnue', async () => {
    const reponse = await souscrireSimule(
      postJson('/api/abonnement-simule', { offre: 'lecture-mensuel', issue: 'gratuit' }, {
        jeton: abonne.accessToken,
      }),
    );
